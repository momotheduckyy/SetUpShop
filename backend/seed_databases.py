#!/usr/bin/env python3
"""
Seed databases with sample data for Render deployment
This matches the local seed.py data exactly
"""
import sys
...
def seed_databases():
    ...
    if existing_users:
        ...
        print("\n✅ Seeding skipped (already done)")
        return

    print("\n📝 Creating users...")
    # Create users (same as local seed.py)
    users = [
    ...
    if not get_equipment_catalog():
        ...
        print("⚠️  Equipment catalog already populated")

    # Add equipment to users
    print("\n🛠️  Adding equipment to users...")
    today = date.today()
